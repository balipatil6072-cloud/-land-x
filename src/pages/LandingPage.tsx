...
export const LandingPage: React.FC = () => {
  ...
  // Full-Width Infrastructure Visual Carousel State
  ...
  const carouselSlides = [
    {
      ...
      badge: 'NATIONAL INFRASTRUCTURE INTELLIGENCE • Nashik, Maharashtra',
      title: 'LAND ACQUISITION',
      subtitle: 'Infrastructure corridor crossing monitored land parcels',
      image: 'https://images.unsplash.com/photo-1500382017468-9049fed747ef?auto=format&fit=crop&w=1600&q=80',
    },
    {
      id: 2,
      tag: 'PROJECT MONITORING',
      badge: 'PROJECT INTELLIGENCE • DMIC Corridor Sector 4',
      title: 'PROJECT MONITORING',
      subtitle: 'Infrastructure development under active assessment',
    ...
    },
    {
      ...
      badge: 'EARLY WARNING RADAR • 618 Corridors Monitored',
      title: 'PREDICTIVE INTELLIGENCE',
      subtitle: 'Early signals detected across acquisition stages',
      image: 'https://images.unsplash.com/photo-1590496793929-36417d3117de?auto=format&fit=crop&w=1600&q=80',
    },
    {
      id: 4,
      tag: 'FIELD VERIFICATION',
      badge: 'CADASTRAL SURVEY INTEGRATION • SLAO Verification',
      title: 'FIELD VERIFICATION',
      subtitle: 'Land and project conditions brought into one intelligence layer',
      image: 'https://images.unsplash.com/photo-1581094794329-c8112a89af12?auto=format&fit=crop&w=1600&q=80',
    },
    {
      id: 5,
      tag: 'OFFICER DECISION SUPPORT',
      badge: 'DECISION SUPPORT • Target Action SLA 3 Days',
      title: 'OFFICER DECISION SUPPORT',
      subtitle: 'From emerging risk to targeted intervention',
  ...
  return (
    <div className="relative w-screen min-h-screen bg-slate-950 text-white font-sans antialiased select-none overflow-x-hidden">
      ...
      <div className="relative z-10 bg-slate-950/95 backdrop-blur-md text-slate-100 w-full space-y-16 py-16 px-4 md:px-12 max-w-7xl mx-auto font-sans text-left">
        ...
        {/* NEW SECTION: FULL-WIDTH INFRASTRUCTURE VISUAL CAROUSEL */}
        <div className="w-screen relative left-1/2 right-1/2 -ml-[50vw] -mr-[50vw] bg-slate-950 border-y border-slate-800 my-4 overflow-hidden select-none font-sans">
          <div
            className="relative w-full h-[320px] sm:h-[380px] lg:h-[480px] overflow-hidden group"
          ...
          >
            ...
            {carouselSlides.map((slide, idx) => (
              ...
              >
                ...
                />
                {/* Subtle Dark Navy Gradient Overlay (~70% visible photo, ~30% overlay influence) */}
                <div className="absolute inset-0 bg-gradient-to-t from-slate-950 via-slate-950/40 to-slate-950/20" />

                {/* Intelligence Slide Content Overlay */}
                <div className="absolute bottom-10 left-6 sm:left-12 max-w-2xl text-left space-y-2 z-20">
                  <div className="flex items-center space-x-2">
                    <span className="px-2 py-0.5 bg-blue-900/90 text-blue-200 border border-blue-700 text-[10px] font-mono font-bold uppercase rounded-xs">
                      {slide.tag}
                    </span>
                  ...
                  <h3 className="text-xl sm:text-3xl font-black text-white tracking-tight font-sans uppercase">
                    {slide.title}
                  </h3>
            ...
            <div className="absolute inset-x-4 sm:inset-x-8 top-1/2 -translate-y-1/2 flex items-center justify-between z-30 pointer-events-none">
              <button
                onClick={() => setCurrentSlide((prev) => (prev === 0 ? carouselSlides.length - 1 : prev - 1))}
                className="px-3 py-1.5 bg-slate-900/80 hover:bg-slate-900 text-slate-200 border border-slate-700 rounded-xs font-mono text-xs cursor-pointer pointer-events-auto transition-colors shadow-lg flex items-center space-x-1"
            ...
            {/* Slide Counter & Dot Indicators Bar */}
            <div className="absolute bottom-3 right-6 sm:right-12 z-30 flex items-center space-x-4 font-mono text-xs text-slate-300 bg-slate-900/80 px-3 py-1 rounded-xs border border-slate-800 backdrop-blur-xs">
              <span className="font-bold text-white">0{currentSlide + 1} / 05</span>
                ...
                {carouselSlides.map((_, idx) => (
                  <button
                    ...
                    className={`w-2 h-2 rounded-full transition-all cursor-pointer ${
                      currentSlide === idx ? 'bg-blue-500 w-4' : 'bg-slate-600 hover:bg-slate-400'
                    }`}
